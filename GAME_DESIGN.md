...
### Core Principle: Specialization + Validation over Monolithic Logic

Monolithic systems fail at precision. One system trying to do everything produces
drift, artifacts, and untraceable errors. Instead, every major system is:

1. **Specialized** — does one thing, knows its domain
2. **Tool-driven** — executes deterministic operations, not fuzzy decisions
3. **Validated** — output is checked against constraints before it commits
4. **Auditable** — state changes are traceable (what changed, what caused it)

This applies to world generation, destruction, enemy AI, and content pipelines.

### The Voxel World as a Non-Destructive Canvas

```
BaseState (generated island)         ← immutable reference — what was there
        ↓
WorldState (current session)         ← working copy — what is there now
        ↓
EditHistory (destruction log)        ← every voxel change, what caused it
```

The island generator produces the BaseState once per seed. All gameplay
operates on WorldState. Destruction is a stream of validated, localized edits —
never a full regeneration. Ancient temple blocks and cursed iron are not
"hard to destroy" — they are formally excluded from the edit set by the
validator (interaction matrix). No code path can bypass this.

### Multi-Agent Pattern — Applied to Game Systems

Each major system maps to an agent role:

| Agent Role | Game System | Responsibility |
|---|---|---|
| Perception | World query / raycasting | What is at this location? What material? |
| Planning | Enemy AI / ability system | What sequence of actions achieves the goal? |
| Tool execution | Destruction, movement, physics | Deterministic operations on world state |
| Validation | Interaction matrix | Did this operation satisfy all material constraints? |
| Memory | Island state, EditHistory | Persistent structured context across frames |

### Content Generation Pipeline

```
[AI / procedural generator]    ← produces colour maps, structure layouts
         ↓
[Perception pass]              ← segments output into material regions
         ↓
[Placement tool agent]         ← applies to voxel grid at correct tier
         ↓
[Validator]                    ← checks against material rules, indestructibles
         ↓
[Committed world state]        ← clean, traceable, constraint-safe
```

Colour maps generated by AI image tools are sampled onto the 0.001 m visual
tier. Gameplay structure (what material, what hardness) lives at the 0.1 m
